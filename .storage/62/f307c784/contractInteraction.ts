...
class ContractInteraction {
  ...
  async register(sponsorCode: number): Promise<{ success: boolean; message: string; txId?: string }> {
    try {
      ...
      const tx = await this.contract.register(sponsorCode).send({
  ...
  async purchasePlan(plan: 50 | 100 | 500): Promise<{ success: boolean; message: string; txId?: string }> {
    try {
      ...
      const tx = await this.contract.purchasePlan(planAmount).send({
        feeLimit: 100_000_000,
      ...
      return {
        success: false,
        message: error.message || 'Error al comprar plan'
      };
    }
  }
  ...
  async getUserInfo(address?: string): Promise<UserInfo | null> {
    try {
      ...
      const info = await this.contract.getUserInfo(userAddress).call();
  ...
  async getUserTransactions(address?: string): Promise<Transaction[]> {
    try {
      ...
      const userAddress = address || this.tronWeb.defaultAddress.base58;
      const transactions = await this.contract.getUserTransactions(userAddress).call();
  ...
  async getUserReferrals(address?: string): Promise<string[]> {
    try {
      ...
      const userAddress = address || this.tronWeb.defaultAddress.base58;
      const referrals = await this.contract.getUserReferrals(userAddress).call();
  ...
  async getGlobalStats(): Promise<GlobalStats | null> {
    try {
      ...
      const stats = await this.contract.getGlobalStats().call();
  ...
  async isUserRegistered(address?: string): Promise<boolean> {
    try {
      ...
      const userAddress = address || this.tronWeb.defaultAddress.base58;
      return await this.contract.isUserRegistered(userAddress).call();
    } catch (error) {
  ...
  async getAddressByReferralCode(code: number): Promise<string | null> {
    try {
      ...
      const address = await this.contract.getAddressByReferralCode(code).call();
