...
/**
 * @title AmigosDeTron - Sistema Correcto con Renovación
 ...
 * - Plan Básico: Cada 5 ciclos, 10 TRX van a la wallet del contrato (renovación)
 * - Plan Master/Premium: Cada 10 ciclos, 10 TRX van a la wallet del contrato (renovación)
 */
contract AmigosDeTronFixed {
    ...
    uint256 public constant RENEWAL_FEE = 10000000;             // 10 TRX
    uint256 public constant BASIC_RENEWAL_CYCLES = 5;           // Cada 5 ciclos
    uint256 public constant PREMIUM_RENEWAL_CYCLES = 10;        // Cada 10 ciclos
    ...
    struct User {
        ...
        // Contadores de ciclos para renovación
        uint256 basicCycles;
    ...
    struct ReinvestmentFund {
        ...
        uint256 renewalFees;  // Acumulado de renovaciones
    }
     ...
     * - 10 TRX al usuario que cicla (EXCEPTO en ciclos de renovación)
     * - 2.5 TRX al sponsor del usuario que cicla
     * - 5 TRX al fondo de reinversión
     * - 5.5 TRX quemados
     * - 2 TRX reserva de gas
     ...
     * 
     * RENOVACIÓN:
     * - Plan Básico: Ciclo 5, 10, 15, 20... → 10 TRX al contrato
     * - Plan Master/Premium: Ciclo 10, 20, 30... → 10 TRX al contrato
     */
    function _processCycle(address _user) private {
        ...
        // 1. Pagar 10 TRX al usuario que cicla (EXCEPTO en ciclos de renovación)
        if (isRenewalCycle) {
            // En ciclo de renovación, los 10 TRX van al contrato
            reinvestmentFund.renewalFees += CYCLE_USER_PAYMENT;
            emit PlanRenewed(_user, renewalPlan, CYCLE_USER_PAYMENT);
        } else {
            // Ciclo normal, pagar al usuario
    ...
    /**
     * @dev Obtiene contadores de ciclos de renovación
     */
