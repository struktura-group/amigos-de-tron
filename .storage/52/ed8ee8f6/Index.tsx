...
import { registerUser, activatePlan } from '@/lib/tronWeb';
...
export default function Index() {
  ...
  const handlePlanPurchase = async (planId: number, planName: string, price: string) => {
    if (!isConnected) {
      toast.error(t('wallet.notInstalled'), {
        description: t('connectWalletDescription'),
      });
      return;
    }
    ...
    try {
      // Si es el primer plan (registro), necesitamos una dirección de sponsor
      // Por ahora usaremos una dirección por defecto o la del contrato
      const sponsorAddress = 'TYourDefaultSponsorAddressHere'; // Actualizar con dirección real
      
      if (planId === 1) {
        // Registro inicial con plan básico
        await registerUser(sponsorAddress);
        toast.success('¡Registro exitoso!', {
          description: `Has activado el plan ${planName} con ${price}`,
        });
      } else {
        // Activar plan superior
        ...
        {isConnected && (
          <Card className="mt-8 max-w-md mx-auto bg-gradient-to-br from-green-900/20 to-green-800/20 border-green-500/30 p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-green-400 font-semibold">{t('walletConnected')}</p>
            </div>
        ...
        <div className="text-center mb-12">
          ...
          {!isConnected && (
            <div className="mt-4 flex items-center justify-center gap-2 text-yellow-500">
              ...
              <p className="text-sm">{t('connectWalletDescription')}</p>
            </div>
          )}
        </div>
            ...
            >
              ...
              <div className="p-6">
                ...
                <Button
                  ...
                  onClick={() => handlePlanPurchase(plan.id, plan.name, plan.price)}
