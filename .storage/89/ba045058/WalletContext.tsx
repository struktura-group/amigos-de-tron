...
interface WalletContextType {
  ...
  basicMatrix: MatrixInfo | null;
  masterMatrix: MatrixInfo | null;
  premiumMatrix: MatrixInfo | null;
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const [basicMatrix, setBasicMatrix] = useState<MatrixInfo | null>(null);
  const [masterMatrix, setMasterMatrix] = useState<MatrixInfo | null>(null);
  const [premiumMatrix, setPremiumMatrix] = useState<MatrixInfo | null>(null);
  ...
  const loadUserInfo = async () => {
    try {
      ...
      if (info && info.isActive) {
        ...
        // Cargar información de matrices
        const basic = await contractInteraction.getUserMatrixInfo(address, 1);
        const master = await contractInteraction.getUserMatrixInfo(address, 2);
        const premium = await contractInteraction.getUserMatrixInfo(address, 3);

        setBasicMatrix(basic);
        setMasterMatrix(master);
        setPremiumMatrix(premium);
      } else {
